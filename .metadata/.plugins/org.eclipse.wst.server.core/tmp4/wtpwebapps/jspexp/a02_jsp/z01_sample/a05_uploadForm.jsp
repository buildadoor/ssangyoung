...
3. 업로드 처리(a06_uploadFile.jsp)
	...
	4) 업로드된 파일 정보 가져오기 
		...
		if(files.hasMoreElements()){
			...
			muti.getFilesystemName(ele); //서버에서 업로드 파일명
			muti.getOriginalFileName(ele); //유저가 업로드한 파일명
			muti.getContentType(ele); //content타입
			muti.getFile(ele).length(); //파일의 길이
		}
	# 이미지 활용 예제 
	1. 회원정보에 사진 이미지 올리기
		회원테이블에 이미지명을 처장하는 컬럼필요
