...
3. request 기본 객체 - 주요 정보 제공 메소드
   ...
   4) getContextPath() : JSP 페이지가 속한 웹 어플리케이션의 컨텍스트 경로를 구한다.
   		wepapp 하위에 절대 기준 경로를 설정할 수 있기때문에 변수를 설정해서
   		공통 css, 공통 javascript, 이미지 폴드, 페이지이동 등을 활용할때 
   		기준경로 설정한다.
...
//경로상 webapp 하위이다.
%>
<img src ="<%=path%>/a00_img/orange.jpg"/>
<!-- 
...
2. 기능메서드
   ...
   request.getParameterMap() : 웹브라우저가 전송한 파라미터의 맵을 구한다. 맵은
      파라미터 이름, 값 쌍으로 구성된다.
      ==> 요청 key를 모르거나, 변동이 되는 경우, 해당 요청 key와 값을 설정하여
      처리할 때 활용된다.
 -->
    <%--
