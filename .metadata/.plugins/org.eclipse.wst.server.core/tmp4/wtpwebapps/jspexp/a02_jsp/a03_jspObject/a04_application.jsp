...
3. 초기 파라미터 설정(web.xml)
	<context-param>
		<description> 파라미터 설명(필수아님)
		<param-name>파라미터 이름
		<param-value>파라미터 값 
	ps) 모든 소스에 특정한 값을 변경을 해야하는 경우..
	파일서버 위치, 특정네트워크를 통해 접속하는 서버의 경로 ..DB 서버 정보.. 
	공통 위치에 키값으로 설정하고 그것만 변경하면 모든 소스를 자동적용되면
	효과적으로 처리가 가능하다. web.xml에서 설정하여 변경함으로
	공통적인 설정값에 대해 효과적인 처리를 할 수 있다.
...
pageContext.setAttribute("name02","박길동(age)");
%>
