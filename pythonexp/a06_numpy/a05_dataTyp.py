...
2. 해당 데일의 type을 확인할 수 있다.
3. astype 함수를 이용해서 타입을 변경할 수 있다.
4. 유형 종류
    정수형 : in8, int16,.. unut8
    실수형 : float16, float32
    복소수 자료 : comple64, complex128
    boolean : bool
    객체 : object
    문자열 : string_
    유니코드 : unicode_
...
ar03 = np.array(["안녕","디지몬","~"]); print(ar03, ar03.dtype)
ar04 = np.array([3.5,4.7,5.2]); print(ar04, ar04.dtype)
ar05 = np.array(["3.5",4.7,5]); print(ar05, ar05.dtype)
# 형변환 처리..
ar06 = np.array([3,2,'4'],dtype=np.int32) #포함된 데이터를 해당 데이터 유형으로 변환 처리 3.4==> 3, '4'==>4
print(ar06, ar06.dtype)
# ndarray 유형을 변환 처리 할 때는 배열명.astype(np.데이터유형)
