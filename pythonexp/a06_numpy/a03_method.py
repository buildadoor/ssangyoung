...
print(ar05)
# 범위에 데이터는 나누어서 처리
# 1. linspace(시작값, 마지막값, 분할할 갯수)
ar06 = np.linspace(0,1,6)
print(ar06)
# 2. np.linspace(시작값, 마지막값, 분할할 갯수, endpoint=False)
#    마지막값이 포함되는 것이 default, 
#    마지막값을 포함시키지 않으려면 endpoint 속성 활용
ar07 = np.linspace(0,10,4, endpoint=False)
print(ar07)
# ex) 0~100 사이의 구간을 5개 구간으로 나누어 출력
ar08 = np.linspace(0,100,5)
print(ar08)
# ex) 1~7 사이의 구간을 6개로 나누어서 출력하되 
#     마지막값 포함하지 않게 처리.
...
print(ar10)
print(ar11)
#ex) 1~16까지 배열 데이터를 4X4 형태의 ndarray로 출력하세요 
ar12= np.arange(1,17).reshape(4,4)
print(ar12)
#ex) 1~21까지 배열데이터 3x7 혈태로 출력하세요  
ar13= np.arange(1,22).reshape(3,7)
print(ar13)
'''
# 초기값이 1이나 0으로 default값을 설정하여 배열크기로 설정하는메서드
1. 1로 채워서 배열을 만드는 경우
    .ones(크기설정)
...
print(np.eye(5, k=-1))
print(np.eye(5, k=2))
# ex) 3x3 형식에에 사선으로 아래로 한칸 밑에 1로 채워지는 데이터를 처리하세요.
print(np.eye(3, k=-1))
'''
# 사선의 데이터를 추출해주는 diag
