...
# 특정 데이터의 삭제
numList03 = [70,80,888,90,100,50,90,80,90]
...
# copy() : 객체의 경우 heap메모리를  만들어 사용하게 처리
numList04 = [4,7,9,8]
numList04.reverse()
...
#     3번째 위치에 임의의 주사위 번호를 삽입
#     역정렬을 시키고, 2번째 데이터 삭제 후 마지막 데이터를 삭제후 
#     데이터 출력
import random as r
diceList = []
for cnt in range(5):
    diceList.append(r.randrange(1,7))
print("5번 던진 후:", diceList)
diceList.insert(2,r.randrange(1,7))
print("5번째 위치 추가:", diceList)
diceList.sort(reverse=True)
print("역정렬:", diceList)
del(diceList[1])
print("두번째 데이터 삭제:", diceList)
diceList.pop();
print("마지막 데이터 삭제:",diceList)