...
import javaexp.a14_review.Worker.FireMan;
import javaexp.a14_review.Worker.PoliceMan;

public class A06_Inheritance {
	public static void main(String args[]) {
		Worker w1 = new PoliceMan(); w1.working(); 
		...
		list.get(1).read();//CookBook내용처리
		for(Book bk:list) { //향상된 for문을 통해 단일 : 다중객체 
			bk.read();	//각각 단위 객체의 read()메서드 호출 처리 
		}
	}
}
		//ex) 상위를 Book() read()책을 읽다
		//		하위 Computerbook() read() 컴퓨터 관련 내용 읽습니다.
...
class ComputerBook extends Book{
	void read() {
		System.out.println("컴퓨터 관련 내용 읽습니다");
	}
}
class CookBook extends Book{
	void read() {
		System.out.println("요리 관련 내용 읽습니다");
	}
}
//Main()상위객체 = 하위객체
...
abstract void working();//하위에서 재정으해서 다양하게 사용할 메서드 
