	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 1. 문자 기반 입력 스트림의 최상위 클래스로 추상 클래스
		 				Reader
		 	FileReader  BufferReader  InputReader
		 	==> 2byte 이상의 문자에 대한 처리 가능(한글 입력이 가능) 
		 2. 주요 기능 메서드
		 	1) int read() : 입력 스트림으로부터 한개의 문자를 읽고 리턴한다.
		 	2) int read(char[] cbuf) : 입력 스트림으로부터 읽은 문자들을 
		 		매개값으로 주어진 문자 배열 cbuf에 저장하고 실제로 읽은 문자 수를 리턴한다.
		 	3) int read(char[] cbuf, int off, int len)
		 		문자의 배열을 입력받았을때 시작 위치와 크기를 지정할때
		 		속성값 매개변수로 설정. 
		...
		//2. 입력한 문자열 갯수 ==> 유동적으로 입력받기 위해서는 BufferedReader가 필요
		BufferedReader bfr = new BufferedReader(reader); //
		/*
		 #Stream 객체는 특정 기능을 추가할때, 
		 상속에 기반하여 하위 객체를 처리하는데, 추가기능 메서드를
		 생성자를 하위객체가 상위 객체를 포함하는 형태로 처리한다.
		 	최상위스트림1
		 	하위스트림2
		 	최하위스트림3
