...
/*
 		...
 		2. 주요 메서드
 			...
 			int read(byte[] b): 입력 스트림으로부터 읽은 바이트들을 매개값으로 
 								...
 								int read(byte[] b, int off, int len): 입력 스트림으로부터 len개 바이트
 									...
 									그리고 실제로 읽은 바이트 수인 len 개를 리턴한다. 만약 len개를 모두 읽지 못하면
 									실제로 읽은 바이트 수를 리턴한다.
 			void close() : 사용한 시스템 자원을 반납하고 입력 스트림을 닫는다.
 */
			//1. 입력 스트림 객체 System.in 통해 객체 생성
			InputStream is = System.in;
			//2. 여러개의 문자열을 입력할 수 잇게 지정 (영문)
			byte [] brr = new byte[10]; // byte 단위의 범위
			//3. read(byte[]) : 해당 배열의 갯수만큼 문자열을 받을 수 있음
			System.out.print("입력할 문자열:");
			try {
				//int cod = is.read(brr);//문자 한 자를 받아서 byte코드할당
