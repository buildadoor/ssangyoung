 		...
 		ex)
 			class Parent {
 				...
 				class Child extends Parents {
 				 //상속을 하는 순간 위에 선언된 부모의 name, all()을 선언한 것과 동일한 효과를 가지고 있다.
 					String gender;
 					void showAll(){
 						call(); //부모의 메서드를 사용
 			...
 			2. 상속 대상 제한
				...
				3) protected : 상속관계에 있으면 해당 접근 제어자로 선언되어 있으면 
					접근이 가능하다(필드/메서드)
 			#extends 키워드
 ...
 */
	//아버지 (모습이 통통하다, running() 빠른 속도로 뛴다), 아들(키가 크다, showAll()부모 구성+1키))
...
class Father{
	private String cha="성격이 온화하다";
	String shape = "모습이 통통하다";
	void running() {
...
class Son extends Father{
	...
	//	System.out.println("성격"+charact); 접근 제어자가 private인 경우 처리하지못함
