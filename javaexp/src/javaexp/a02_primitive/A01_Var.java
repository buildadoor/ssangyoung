...
//		int tot = num01 + 10; 초기화되지 않아서 에러를 발생
	...
	반복문/조건문, 클래스/메서드에서 추후 언급
	2. 즉, 상위단계에서 선언된 변수는 하위에서 호출하여 사용되고,
	3. 하위 단계에서 선언된 변수는 상위에서 선언 및 호출하지 못한다.
	
	#primitive 데이터 타입
	1. 자바에서는 기본 stack영역에서 선언 및 호출, 할당되는 기본 데이터 타입으로 정수, 실수, 문자, 논리형이 있다.
	ps) 자바는 크게 메모리를 stack, heap으로 나누어서 사용한다.
		작은 메모리인 기본 데이터 유형은 stack영역에
		선언과 저장을 동시에 할 수 있고,
		이런 기본 유형을 모아서 리스트한 배열이나, 
	...
	3. 정수형
		...
		char	2byte	0~65535
			...
			'A'==> 문자
			 0 ==> 숫자
			'0'==> 문자 
			char 유형은 int유형으로도 변경이 가능하고,
			int 유형은 casting에 의해 문자한자로 변경이 될 수 있다.
		short	2byte
		...
		long	8byte
			...
			정수형 기본 유형이 아니기에 L,1를 붙여야 사용할 수 있다.
			23423423423423L
...
//		num01 = 257; 범위가 넘어 에러 발생
		...
		//3. 정수형 변수를 선언하고, 해당 정수의 코드값에 해당하는 유니코드를 확인하려면, casting으로 확인할 수 있다.
		int code04 = 70;
		...
		/*
		 4. 실수형
			1)float : 4byte
			2)double: 8byte (실수형의 default데이터 유형)	
	
		 * */
		double d01 = 2.242;
		float f01= 3.42F; //float 유형일 때, F를 붙여서 사용한다.
		/*
		 5. 논리형 
		 	1) boolean
		 		true/false
		 ...
		 */
		int point=90;
		boolean isPass = point>=80; //80이상일때 합격 
		...
		/*
		 ...
		 10년 후 나이를 처리하고, 초기화한 후 10년 후 나이를 처리하고 
		 출력하세요.
		 ex2)문자 'Z'.'홍'의 유니코드값을 확인하여 출력하세요.
		 ex3)점수 데이터 100에 해당하는 문자를 출력하세요.
		 */
		...
		int code08 = letter08;
		int code09 = letter09;
		System.out.println(code08);
		System.out.println(code09);
		//3번 
		int code07 = 100;
	...
	//1번
		int age1;
		//int age110 = age 1 + 10; 초기화 되지 않을때 에러발생
		int age=25;
		int age10 = age + 10;
