 ...
 */
	...
	/*
	 2. substring("추출할 시작위치(0~), 추출할 마지막위치(1~)
	 	특정한 문자열에서 문자열을 추출하여 문자를 만들때 사용한다. 
	 */
	String subStr = target.substring(0,3);
	System.out.println("추출한 문자열");
	System.out.println(subStr);
	/*
	Scanner sc = new Scanner(System.in);
	System.out.println("주민번호앞자리를입력하세요(YYMMDD)");
	...
	if(civilNum.length()==6) {
		...
		System.out.println("입력 형식은 6자리 입니다");
	}
	*/
	/*
	 3. split("구분자"): 특정 문자열을 구분자에 의해 구분해서 구분된 데이터를 배열로 할당 처리 
	...
	//ex) 서울-대전-대구-부산과 같이 나열된 문자를 split구문으로 
	//만들어 향상된 for을 통해서 출력하세요  
 ...
 */
	...
	//ex)StringTokenizer 객체를 이용하여 사과@3000@2개를 
	//for/while문 하나씩 예제를 통해 출력하세요 
	...
	for(int cnt1=1;cnt1<=st2.countTokens();cnt1++) {
	//바로 입력시 countToken()갯수가 줄어들어
		//3번까지 반복처리 못함.. 
	System.out.println(st2.nextToken());
	}//데이터를 초기화 처리가 필요하다 .
	System.out.println("#while#");
	st2 = new StringTokenizer("사과@3000@2개","@");
