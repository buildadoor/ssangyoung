...
/*
 ...
 3. 주요 클래스
 	Object - 자바 클래스의 최상위 클래스로 사용
 			...
 			2) 다형성에 의해서 모든 객체는 Object 객체에 할당이
 				가능하다.
 				Object ob = new Person()
 			3) 그래서 상위에 재정의하는 메서드는 상위 bject의
 				참조변수로 사용이 가능하지만 
 			4) 하위 고유의 메서드를 사용하고자 할때는 typecasting이
 				필요로 하다.
 				ex)
 				Person 클래스에서만 선언된 name필드와
 				getName()메서드를 상위 클래스의 참조변수
 				ob로 사용 불가
 				ob.getName() (X)
 				타입 캐스팅을 하여야한다.
 				Person p = (Person) ob;
 			...
 			타입 캐스팅하는 불편함이 있는데, 단일 객체를 사용할 경우 
 	...
 	String - 문자열을 저장하고 여러가지 정보를 얻을때
 		"문자열"+"문자열" : 메모리 2개 
 	StringBuffeer,StringBuilder
 		-문자열을 저장하고 내부 문자열을 조작할때 사용
 		sbf.append("문자열")
...
/*
 ...
 3. 기본적으로 == 연산자와 동일한 결과 리턴(반지 비교)
 4. 논리적 동등 위해 오버라이딩 필요
 	1) 논리적 동등이란?
 		-같은 객체이건 다른 객체이건 상관없이 객체 저장 데이터 통일 
 ...
 5. 객체 해시코드(hashCode())
 	...
 	2) 논리적 동등 비교시 hashCode() 오버라이딩의 필요성
 		컬렉션 프레임워크의 HashSet. HashMap, HashTable과
 		같은 클래스는 두 객체가 동등한 객체인지 판단할때 사용된다.
 5. 참조 변수의 기본 문자열 출력 형태 선언 (toString())	
 	*
 	*	
 */
		Member m01 = new Member("홍길동");
		Member m02 = new Member("김길동");
		Member m03 = new Member("마길동");
		System.out.println("비교 연산자:"+(m01==m02));
		System.out.println("euqals 메서드:"+(m01.equals(m02)));
		System.out.println("비교 연산자:"+(m01==m03));
		System.out.println("euqals 메서드:"+(m01.equals(m03)));
}
...
class Member{
	...
	public boolean equals(Object obj) {//Object obj = new Member "홍길동 
		...
		Member m = (Member)obj;
		//해당객체의 속성을 사용하려면 type casting필요
		//문자열로 name 속성이 같을때, true 그 외에는 false 
