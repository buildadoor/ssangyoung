...
//		System.out.println(p01);
		...
		//2. 객체 유형 : null이라는 키워드로 stack영역에 초기화를 선언 - int[]arry01=null
		//		주의 - 아직까지 heap영역에 실제 객체를 할당하지 않았음 의미.
		/*
		 
		 */
		System.out.println(p01);
		//초기화를 했지만, 식제 객체릃 할당하지 않았음에도 불구하고, 실제 객체가 있는 것과
		//동일하게 구성요소를 호출할 때 발생 
		//ex)자동차가 아직까지 주차되지 않은 상태에서 자동차의 트렁크 내용을 확인하려고 함 
		//NullPoninterException
		//Pointer : heap 영역에 주소를 지칭
		//Null : 실제 객체가 없음을 의미 
		/*
		 if(p01!=null){객체가 생성되어 heap영역에 주소에 있을때,
		 	p01.name
		}
		 */
...
//		호출하면 runtime error인 NullPointerException 발생
