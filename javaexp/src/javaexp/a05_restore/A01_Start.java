	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 2. 하나의 공간 (작은 공간) -stack영역
		 	...
		 	- 크기가 일정하지 않음 (객체형 데이터 - 배열,내장된 객체,사용자 정의 객체)
		 	ex) 도서관/서점에서 물건 검색하는 단말기 : 해당 도서의 위치를 가르켜준다.
		 3. 두번째 공간 (큰 공간-방1, 방2......) -heap영역
		 	1) 실제 큰 물건들이 있는 공간
			ex) 단말기를 통해 저장한 위치를 찾아가서 실제 도서가 있는 것을 확인.
 		# 자바의 데이터 타입 분류 
 		1. 기본 타입 
 		2. 참조 타입
 		- 자바는 정수/실수/boolean형을 바로 선언하여 사용하는
 			기본형 타입과, 객체형 타입(배열/열거/클래스/인터페이스)에 의해
 			특정한 메모리를 참조하는 참조타입으로 크게 분류할 수 있다.
 		# 변수의 메모리 사용
		...
		//	다르기에 다른 데이터가 나타난다.
		int num03 = num01; //num01의 데이터를 num03 핳당. 여기에 바나나3가 있습니다. 
		//바나나라고 저장된 실제 바나나를 할당
		System.out.println(num01+":"+num03);
		num03 = 20;//바나나3이라고 이름 붙인 곳에 바나나 주스를 위치 시킴
		System.out.println(num01+":"+num03);
		// 객체의 경우..
		int[] arr03 = arr01; 
		//메모리의 주소를 할당하기 자동차3가 있습니다. 여기에 자동차가 있습니다.라고 적힌 곳의 위치 정보를 복사해서 할당.
		//이 정보가 (주차구역1 12자리) 자동차3에 할당
		// ex) 같은 위치에 있는 집에 홍길동(아버지)의 집.
		//						이영자(어머니)의 집
		System.out.println(arr03+":"+arr01);
		System.out.println(arr03[0]+":"+arr01[0]);
		arr03[0] = 20;//해당 위치에 있는 자동차에 사과 1박스를 트렁크에 위치 시킴.
		System.out.println(arr03[0]+":"+arr01[0]);
		//같은 주소에 있는 데이터를 이름만 달리하고 호출하기에
		//데이터를 동일하게 가지고 있다..
		//비교연산자의 비교
		//기본 유형의 비교는 할당된 실제 데이터를 비교.
		//*자바의 데이터의 비교 연산자는 stack영역의 데이터를 비교하는 것이다.
		//1. 기본 데이터 유형 - 실제 데이터를 확인
		//2. 객체 유형 - heap영역에 주소가 같은지 여부를 확인
		System.out.println("#기본 유형 데이터의 비교#");
		...
		//ex2) int train[]={3}을 3개 train2, trian3을
