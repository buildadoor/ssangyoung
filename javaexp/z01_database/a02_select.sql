 ...
 */
--where 조건 처리
SELECT deptno, ename, sal --[열단위 선택] 컬럼단위로 전체 행을 선택 
FROM emp 
WHERE sal >= 3000; --[행단위 filtering]sal가 
--group 함수 처리
SELECT deptno, SAL 
FROM emp
ORDER BY deptno;
...
--sal(급여),deptno(부서번호)
--ex1) 직책이 'SALESMAN'인 사원정보를 출력를 출력하세요
...
/*
 #테이블에 ALIAS를 사용하는 경우
 1. 두개 이상의 테이블을 사용하는데, 두대 테이블에 공통 컬럼이 있을 경우
 	해당 컬럼명 앞에 e.컬러명, d.컬럼명으로 어디 소속의 테이블인지 지정할 때
...
SELECT *
FROM emp 
WHERE deptno <> 20; -- <>도 같지 않을때에 사용된다
SELECT *
FROM emp 
WHERE NOT (deptno = 20); -- not(조건문)으로 반대조건..
SELECT *
FROM emp 
WHERE not(deptno = 20 AND deptno = 30);
--	부서번호가 20이고 30인 경우가 아닐때 부서번호가 20이 아니거나 30이 아닌 경우
--  deptno != 20 or deptno <> 30 -드모르강 법칙
SELECT *
FROM emp;
-- ex1) 부서번호가 30이 아닌경우 처리를 위의 연산자 내용으로 3가지 기술하세요
SELECT *
FROM emp 
WHERE deptno !=30;
SELECT *
...
--ex3) 급여가 1000미만이거나 4000 이상이 아닌 경우를 출력하세요
SELECT *
FROM emp 
