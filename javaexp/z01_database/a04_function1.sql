...
1. sql에서 함수 (주로. 내장함수)
	...
	2) 숫자 또는 날짜 데이터의 출력 형싱이 변경하는 경우이다.
...
4. 단일행 함수의 사용법
	...
	where 컬럼명 = 함수명(컬럼명/데이터, 매개변수1, 매개변수2...)
 	...
 	select ename 
 	from emp
 	where deptno = 7710; --deptno는 안나오고 ename만 출력
 	select upper(ename) --출력할 내용 (HIHI) 
 	from emp
 	where lower (ename) = 'hihi'; --조건을 처리할 내용 (대문자로 출력)
 	HiHi, HIHI, hihi ==> hihi
 */
/*
 #문자함수
...
--		The job of [Smith] is a [Clerk]!!! [컬럼]
SELECT 'The job of ['||initcap(ename)||'] is a ['||initcap(job)||']!!!'
FROM emp;
/*
 # lower(), upper()의 사용
 1. 입력되어있는 데이터를 대소문자 상관없이 검색을 처리할 때, 활용
 2. 'a'문자로 입력하거나 'A'를 입력해서 검색하더라도 대소문자 상관없이
 	처리가 필요할때, 위 함수를 처리한다. 
 */
--o/O를 입력해서 O문자가 포함된 사원명을 검색하고자 할 때
SELECT ename, job
FROM emp 
...
-- 조회 조건의 컬럼명도 대문자로 변환 후 검색하여야한다.
--ex) 사원정보 테이블에 직책이 'Man'|'MAN'|'man'등 여러형식으로
-- 있을 지라도 대소문자 관계없이 키워드 검색되게 하려면 사용하는 sql을 처리하세요
SELECT job, ename, lower(JOB), upper(JOB)
FROM emp;
SELECT *
FROM emp 
WHERE lower (job) LIKE '%'||lower('MAN')||'%';
--모두 다 lower로 사용하면 검색을 할 수 있다.
/*
 #문자열 길이 변환
 1. length(데이터/컬럼명) : 입력된 문자열의 길이 (글자수)를 반환하는 함수
 2. lengthb(데이터/컬럼명) : 입력된 문자열의 바이트를 반환하는 함수
...
SELECT 'hello' 인사1, length('hello') 인사1크기1, lengthb('hello') 인사1크기2,
		'안녕'인사2, length('안녕') 인사2크기1, length('안녕') 인사2크기2
		FROM dual;
SELECT ename, length(ename) 사원명길이
FROM emp;
---ex) 사원 테이블에서 사원명과 직책의 글자 수를 포현하되, 직책이 6이상인 데이터만
--		출력하세요
