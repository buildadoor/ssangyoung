...
GROUP BY deptno
ORDER BY deptno;
--ex) 직책별과 사원수의 수의 평균 급여를 출력하세요... 
SELECT  job, count(job) 사원수, round(avg(sal)) 평균급여
FROM emp
GROUP BY job;
--날짜형 데이터 min, max()는 최근 데이터, 가장 먼저데이터 
SELECT deptno, min(hiredate) "가장 먼저 입사", max(hiredate) "가장 나중"
FROM emp
GROUP BY deptno
ORDER BY deptno;
--부서별로 평균 급여 중에 조건 검색
...
GROUP BY deptno 
HAVING avg(sal) >= 2000; 
--적용한 그룹 함수의 조건을 처리할 때는 having을 활용한다
...
2. 기본형식
	...
	group by 그룹할 컬럼/데이터
	having 그룹함수 () >=0;
		having은 그룹함수에 비교, 조건/논리 연산식에 붙여 처리한다.
 */
-- 분기별 최대 급여액 출력
...
SELECT to_char(hiredate,'YYYY') 입사연도, 
		count(to_char(hiredate,'YYYY'))"연도별 사원수", 
		max(sal) 최대급여
FROM emp  
GROUP BY to_char(hiredate,'YYYY')
ORDER BY 입사연도; --ORDER BY는 alias이름으로 지원 처리
