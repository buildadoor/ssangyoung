 	...
 	2) 명시적 데이터 타입의 변환
 		...
 		ex) 현재 컬럼 데이터 유형을 변환해서 조건에 맞게 처리하는 경우
 			...
 			해당 컬럼에 맞게 데이터 유형을 변환해서 처리하는 경우
 				...
 				insert emp(hiredate) values(to_date('2022-01-12','YYYY-MM-DD'))
 		...
 		-to_date(컬럼/데이터) : 문자/숫자 타입을 날짜로 변환해주는 함수
 			입력데이터(문자열)기준으로 날짜형으로 변경해서 처리하고자 할 때..
...
--형식을 기준으로 검색이 가능하다. 
SELECT * FROM emp;
--ex)12월에 입사한 사원을 묵싲거 형변환에 의해 검색하세요
...
/*
 ...
 3. 출력할 형식
 	...
 	7) DDD/DD/D : 날짜 표기
 	8) DY/DAY : 요일 표기
 	위의 있는 형식에 한글 등 특수문자를 혼합해서 표기할 떄는 
 	'YYYY"년도"MM"월"' 형식으로 "추가문자열"을 표현한다.
 */
SELECT ename, hiredate, 
		to_char(hiredate,
		'CC"세기" AD YYYY/MM/DD Q"/4분기" MM"월" WW"째주" DAY') show,
...
FROM emp;
--ex) 사원명은 @@년 @@월에 @@/4 분기에 입사했습니다.
SELECT ename||'는 '||
		to_char(hiredate,'YY"년" MM"월에" Q"/4 분기 "')
		||'입사했습니다' show
FROM emp;
--ex) 1/4분기에 입사한 사원의 이름과 입사년월일을 출력하세요
...
--ex) sysdate를 활용해서 현재 시간을 @@시@@분@@초 (24시)표시 형태로 출력하세요
SELECT sysdate, to_char(sysdate,'"현재시간은" HH24"시"MI"분"SS"초"') show FROM dual;
/*
