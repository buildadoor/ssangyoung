...
FROM dual;
-- ex) 분기별 보너스를 급여 기준으로 
--		1사분기 100%. 2사분기 200% 3사분기 300% 4사분기 400$
--		로 처리하기로 했다.사원명,급여,보너스를 출력하세요
SELECT ename, sal, to_char(hiredate,'Q')"입사분기",
		to_char(hiredate,'Q')*100||'%' "보너스%",--묵시
		sal*to_char(hiredate,'Q')"보너스",--묵시
		sal+  sal*to_number(to_char(sysdate,'Q')) "급여 +보너스"--명시 
FROM emp;
/*
 #to_date : 날짜형으로 변환
...
FROM dual;
--ex) 2022-12-31이 현재로부터 남은 날짜 수를 to_date함수로 처리하여 출력하세요
SELECT floor(to_date('2022-12-31','YYYY-MM-DD')-sysdate) "D-day" FROM dual;
--ex) hiredate에서 입사년도만 현재년도로 바꾸어서 날짜 데이터를 만든 후 사원명, 입사일
--		을 출력하세요
SELECT ename, 
...
/*
 ...
 1. 가장 내부에 있는 함수 f1이 결과값을 다음 함수f2에 인수로 적용하고,
...
FROM emp;
--ex) comm이 null일때는 sal의 15%로 설정하고, null이 아닐떄는 
	--	comm에서 sal의 5%를 더하여 보너스 금액을 처리하세요 10단위 절삭처리
SELECT ename, sal, comm, trunc(nvl2(comm,comm+sal*0.05,sal*0.15),-1) 보너스
FROM emp;
/*
# nullif(데이터1,데이터2)
...
-- 600 그대로 지급, 그렇지 않으면 comm으로 지급 처리 
...
--ex) 사원번호 기준으로 짝수면 홀팀, 홀수이면 청팀으로 출력하세요.
-- 사원명, 사원번호, 구분자, 팀
...
--	이름, 급여, 보너스%, 보너스 clerk salesman president manager analyst
--  조건 처리시 일단, 항목이 규칙성이 있는지 여부를 확인한다. (/,mod,*...사칙)
--  위 연산식으로 최소한 범위로 만든 후 , decode나 case문이 필요할때, 활용한다
 	...
 	1) 조건문 형태의 활용
 		...
 		주의 case when 구문의 앞에 선언 조건식을 제외한 조건을 처리한다.
 			if (point >=90){
 				...
 				// else에서 point < 90 내용을 포함하기에 필요 없음. 
 			}
...
SELECT ename, sal, 
	CASE WHEN sal>=5000 THEN 'A등급'
		 WHEN sal>=4000 THEN 'B등급' -- sal<5000 AND 포함되어 있기에 필요 XXXX if랑 쓰지마 XXX
