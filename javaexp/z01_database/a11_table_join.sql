/*a11_table join
 # 조인(join)
 1. 조인의 개념
 	1) 하나의 sql명령문에 의해 여러 테이블에 저장된 데이터를 한 번에 
 		조회할 수 있는 것을 말한다.
 	2) 관계형 데이터베이스 분야의 표준
 		entity relation diagram(ERD)
 	3) 두개 이상의 테이블을 결합한다는 의미 
 2. 조인의 필요성
 	1) 조언을 사용하지 않는 일반적인 예
 		학생 주소록을 출력하기 위해 학생들의 학번, 이름, 소속학과
 		이름을 검색
 		핛애에 대한 정보 검색하는 단계 필요
 ...
 **/
SELECT * FROM emp;
SELECT * FROM dept; -- deptno로 연관관계가 설정
--7369 사원의 부서정보 가져오기
SELECT * 
FROM emp 
WHERE empno = 7369;
...
SELECT ename, sal, dname, e.deptno -- 공통컬럼만 alias이름 선언 
FROM emp e, dept d
WHERE e.deptno = d.deptno 
AND sal >= 3000;
...
FROM emp e, dept d
WHERE e.deptno = d.deptno
--조인을 하면 해당 조인된 구문에 의해 나타낼 데이터를 테이블 형식으로
...
/*
 #Non equi join
 1. 조인하는 두개의 테이블 값이 동일하지 않고, 범위로서 join하는 경우를 말한다.
 */
...
--ex)4등급에 해당하는 사원들의 정보를 출력
SELECT s.grade, e.* 
FROM emp e, salgrade s 
WHERE sal BETWEEN losal AND hisal
AND grade =4;
--ex)SMITH의 급여와 급여등급을 출력
...
/*
 ...
 3. 기본 형식 
	1) (+) 기호를 사용하여 해당 테이블에 정보가 없을지라도 표시할 컬럼을 지정하여 처리한다.
...
/*
# 데이터가 없는 테이블의 outer join시 처리된 내용
1. nvl 함수처리
	없는 데이터에 대하여 nvl함수를 통해 데이터가 없을때 표기 
	ex) 합산 할 때, 없는 데이터도 통계치에 포함될 때는 nvl()을 활용하여
		처리한다.
...
--것을 0으로 처리해준다.
SELECT dname, max(nvl(sal,0)) 부서별최대,
	 count(e.deptno) "사원정보의 건수", count(d.deptno)"부서정보"
FROM emp e, dept d
WHERE e.deptno(+) = d.deptno
GROUP BY dname;
