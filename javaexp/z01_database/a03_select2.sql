/*
 ...
 2. 처리 내용
 	...
 	2) 함수 처리 nvl
 		...
 		숫자형 nvl(comm,0)
 		문자형 nvl(ename,'') 
 		comm이 숫자유형의 데이터이기에 초기값을 0으로 설정한ㄷ.
 		만약에 ''을 넣으면 에러 발생.
 		
 	ps) 주의
 	1. null은 연산 처리가 되지 않기에 연산 처리나 조건문에 활용할때 nvl을 활용한다.
...
--부서 번호가 10이거나 20이거나 30인 경우 출력
SELECT ename, deptno
FROM emp 
WHERE deptno in(10,20,30);
--ex) 직책이 'SALESMAN'이거나 'MANAGER'인 경우에 사원정보를 출력하세요.
SELECT *
FROM emp
...
SELECT *
FROM emp 
WHERE ename LIKE '%N'; --N으로 끝나면 검색
SELECT *
FROM emp 
WHERE ename LIKE '__A%'; --3번째 자리가 A이면 검색

--ex1) 직책에 'MAN'이라는 글자가 포함되어 있으면 
SELECT *
FROM emp 
WHERE job LIKE '%MAN%';
--ex2) 사원명에 S로 끝나는 글자와 
...
FROM emp 
WHERE ename LIKE '%S%';
--ex4)사원명이나 직책에 마지막 두번째 글자가 'E'가 들어가는 데이터 조회
SELECT *
FROM emp 
WHERE ename LIKE '%E_' OR job LIKE '%_E';
/*
 #정렬처리
 1. sql 명령문에서 검색된 결과는 데이터가 입력된 순서대로 출력
 2. 데이터의 출력 순서를 특정 칼럼을 기준으로 오름차순/내림차순으로 정렬하는 경우가 발생한다.
 3. 여러개의 컬럼에 대해 정렬 순서를 지정해서 처리해야 할 경우가 발생한다.
  	- 입사일은 오름차순, 급여 기준으로 내림차순
 4. 기본적인 정렬 방법
 	- 문자값을 알파벳순으로 출력되고, 한글은 가나다순으로 출력된다.
 	- 숫자값은 가장 작은 값으로부터 출력
...
ORDER BY empno desc;
--ex)입사일을 기준으로 최근에 입사한 사람으로 제일 위로 가장 먼저 입사한 사람을 뒤로 정렬
SELECT hiredate, ename
FROM emp 
ORDER BY HIREDATE desc;
--ex)연봉이 가장 높은 사람부터 가장 낮은 사람으로 정렬
SELECT sal, ename
FROM emp 
ORDER BY sal desc;
--ex)부서번호는 오름차순, 연봉은 내림차순으로 출력되게 처리
SELECT deptno, sal
FROM emp 
