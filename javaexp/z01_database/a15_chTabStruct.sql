...
-- 		해당 테이블에 bonus컬럼 default 데이터 0을 입력처리하세요 
CREATE TABLE emp15 
...
--2개 이상의 컬럼을 추가할때는 add( 컬럼1, 컬럼2, 컬럼3) 형식으로 처리
/*
# 컬럼의 삭제
1. 기본 형식
	alter table 테이블명 drop culmn 컬럼명;
 */
SELECT * FROM emp15;
--직책 삭제
ALTER TABLE emp15 DROP COLUMN job;
--ex) emp16으로 사원정보와 부서정보가 조인된 컬럼을 만들고, deptno를 삭제
CREATE TABLE emp16
...
1. 기능
	...
	3) 데이터 유무에 따른 한계
		...
		- 데이터가 있는 경우 : 
				타입변경은 같은 유형까리 변경하되 작은 유형에서 큰 유형으로 변경가능하다.
...
ALTER TABLE emp17
MODIFY hiredate varchar2(16); --데이터가 있으므로 에러발생
ALTER TABLE emp17
MODIFY ename varchar2(50);-- 같은 데이터 유형을 크게 변경하는 것은 가능
--ex) emp18로 데이터가 없는 복사테이블 생성, 사원명을 날짜 유형으로 변경
CREATE TABLE emp18 AS SELECT * FROM emp WHERE 1=0;
ALTER TABLE emp18
MODIFY ename DATE;
SELECT * FROM emp18;
--ex) emp19로 데이터가 있는 복사 테이블 생성 직책과 이름을 varchar(50)으로 각각변경
