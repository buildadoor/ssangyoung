/*
 ...
 2. 종류
 	...
 	2) trunc : 지정한 소숫점 자리까지 남기고 절삭처리
 				trunc(데이터, 자릿수) 
 				자릿수는 round와 동일하게 해당 자릿수만큼 절삭처리해준다. .
 	...
 	4) floor(데이터) : 해당 데이터를 정수형으로 내림 처리한다.
				ex) mod(), /,*, floor/ceil : 중첩함수를 이용해서 다양한 데이터의 범위와
			 					...
			 					0~999, 1000~1999
			 					trunc(특정수/1000) : 0 (0~999), 1(1000~1999), 2(2000,2999),
												: 연속된 범위로 조건 데이터 처리시
								mod() : 반복된 조건의 데이터를 처리할 때, 사용 
									홀짝, 3의 배수 (0,1,2), 5의 배수...
	ps) 날짜형의 경우에서 중첩함수로 위 숫자형 데이터에 대한 절삭이나 올림을 처리하는 경우가 많다.
	ex)	날짜 +1 : 1일, 날짜 + 0.5 = 1일 12시간 
...
/*
#mod
1. 데이터베이스에서는 산술연산자 %(나머지)가 없어 난머지 처리 함수를 사용한다.
2. 형식
	mod(데이터,m) : 데이터를 m으로 나눈 나머지값 
	ex) mod(10,3) ==> 1
 */
SELECT mod(10,3) d1, mod(10,2) d2, mod(10,3) de FROM dual;
