/*
 # 전날 sql파일 처리
 1. 파일에 내용을 적고, 실행하는 것을 기존에 연결 상태로 계속 할 수 있다.
 2. 저장을 누르는 순간 연결이 끊어진다.
 	->파일에 변경된 내용은 마지막에 저장을 누른다
 	->중요한 부분에서 저장을 한 번 하고, 연결 옵션을 다시 선택
 */
...
/*
블럭후, ctrl+shift+/여러줄 주석 
# 데이터 조회하기
데이터는 select 구문을 이용하여 저장된 데이터를 호출할 수 
있다.
1. 기본 형식
	select * | 컬럼1, 컬럼2
	from 테이블명 
...
FROM emp;
-- emp 테이블의 ename, job, deptno컬럼을 검색
--
--ex1) dept 테이블의 전체 컬럼 검색

SELECT * 
FROM dept;
--ex2) emp테이블의 empno, mgr, hiredate, deptno 출력
...
FROM emp;
/*
 #기본적인 테이블 처리 문법은 여러 테이블과 함께 사용할때
 활용된다. 통일한 컬럼이 있을 경우 구분하기 위한 내용이다.
 테이블이 하나밖에 없을때는 테이블명을 생략하여도 된다.
 */
SELECT * FROM emp;
SELECT * FROM dept;
SELECT emp.deptno,	dept.deptno, loc 
FROM emp, dept ; 
...
SELECT deptno, dname
FROM dept;
--그런데, 특정 컬럼을 나타내고 전체 컬럼을 나타낼때는 alias를 
...
-- sal와 comm을 합산하여 아래의 컬럼 형태로 나타내세요
-- 사원번호()
