...
<%-- 
	클라이언트(브라우저) <====> 서버(톰캣) 
	1. 웹서버가 고정 아이피(도메인주소), 서버를 프로젝트단위 소스와
		실행을 한 상태.. (준비가 된 상황) - 마트에 손님 받을 준비
	2. 클라이언트 url 주소로 도메인 => ip 주소
		==> port (마트에 여러물건을 파는 코너가 있듯이 웹서비스1,
		웹서비스2,.. 오라클서버)
	...
	3. 서버가 해당 자원(html/jsp) client에 전달한다.
		...
		# 클라이언트로 전달되기 전에
		1) html(태그, css, js): 바로 전달 
		2) jsp (html + java) ==> java ==> calss ==> 
	...
	5. 클라이언트 : form 형식이 있는 내용을 보고, 데이터를 입력 후, submit를 클릭하면
		...
		페이지명?요청키=요청값
	6. 서버는 해당 자원의 경로에 따라서 받고, 요청값이 있는 경우는
...
3. jsp 페이지를 form으로 만들고 submit 처리하면 다시 현재 페이지호출
	...
	4. 서버에서는 요청값을 받아서 해당 내용을 실행시키고, 그 결과를 html을 만들어서 다시 
		client에 전송한다. ${param.id}
	5. 브라우저(클라이언트)에서는 이 내용을 최종 결과로 확인할 수 있다.
